...
异常得分最高的前 K 个样本中命中的正例数 / 全集中正例数。
当 ``y_true`` 无正例时返回 0.0。

核心组件:
    - RecallAtKConfig: 配置类（继承 BaseMetricConfig）
    - RecallAtKMetric: Recall@K 指标算子

使用示例::

    from tsas.engine.operator.evaluation import RecallAtKMetric

    op = RecallAtKMetric()
    result = op.run((y_truth, y_score))

    op = RecallAtKMetric(k=50)
    result = op.run((y_truth, y_score))
"""
...
class RecallAtKConfig(BaseMetricConfig):
    """Recall@K 评价指标配置

    Attributes:
        k (int): 取 top-k 个样本，默认 10
        pos_label (int): 正例标签值，默认 1
        main_scores (dict[str, str] | None): 主评分路径映射，默认 ``{"ratk": "_"}``
    """
    model_config = ConfigDict(frozen=True)

    k: int = Field(default=10, ge=1, description="取前 k 个最高分样本计算 recall")
...
class RecallAtKMetric(
    ...
    取 ``y_score`` 最高的前 ``k`` 个样本，统计其中等于 ``pos_label`` 的数量，
    并除以 ``y_true`` 中等于 ``pos_label`` 的总数量。
    无正例时返回 0.0；``k`` 越界时抛出 ValueError。
    ...
    Output:
        float: Recall@K 值，范围 ``[0, 1]``

    泛型参数:
        I: tuple[np.ndarray, np.ndarray] — (y_truth, y_score)
        MR: float — 标量 Recall@K
        MC: RecallAtKConfig — 配置类
        RP: None — 无运行参数
    """
    ...
    def _run(
        ...
        *,
        ...
        return float(np.sum(top_true == pos_label) / n_pos)