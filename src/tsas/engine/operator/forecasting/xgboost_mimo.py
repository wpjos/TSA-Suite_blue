...
基于 EPFAlgorithm 中的 XGBoostMIMO 模型思想，采用 MIMO（Multi-Input Multi-Output）
多步预测策略：使用 XGBoost booster 同时输出所有未来步长的预测值。
...
class XGBoostMIMOForecasterConfig(BaseModel):
    """XGBoost MIMO 预测算子实例参数。
...
class XGBoostMIMOForecaster(BaseForecaster[ForecastExtraOutput,
    ...
    def __init__(self, *, oid: str | None = None, config: XGBoostMIMOForecasterConfig | None = None, **kwargs):
