...
采用 MIMO（Multi-Input Multi-Output）思想
多步预测策略：使用单个 LightGBM booster 同时输出所有未来步长的预测值。

实现方式：
- 将预测步长索引（0..pred_len-1）作为额外特征加入每个样本。
- 每个目标维度训练一个 booster；该 booster 以“窗口特征 + 步长索引”为输入，
  预测对应步长的目标值。
- 推理时构造 pred_len 条样本（步长索引分别为 0..pred_len-1），一次 predict
  得到完整多步预测。
...
class LightGBMMIMOForecasterConfig(BaseModel):
    """LightGBM MIMO 预测算子实例参数。
...
None]):
    ...
    为输入，同时预测所有未来步长。特征由输入历史窗口展平得到，适用于通用数值时序。
    ...
    def __init__(self, *, oid: str | None = None, config: LightGBMMIMOForecasterConfig | None = None, **kwargs):
    ...
    def _build_samples(self, x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        对每个有效起始时刻 t 和每个预测步长 k，构造一条样本：
        特征 = [窗口展平特征, 步长索引 k]，目标 = y[t + seq_len + k]。
