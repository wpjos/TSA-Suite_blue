...
- ``strategy=None``（默认，Direct）：为每个预测步长（以及每个目标维度）
  独立训练一个 LightGBM booster。
- ``strategy='MIMO'``：每个目标维度训练一个 LightGBM booster，以
...
class LightGBMForecasterConfig(BaseModel):
    """LightGBM 预测算子实例参数。
    ...
    # ---- LightGBM booster 超参数 ----
    num_leaves: int = Field(default=31, ge=1, le=1024, description="每棵树最大叶子数")
    ...
    min_child_samples: int = Field(default=20, ge=1, le=10000, description="叶节点最小样本数")
...
class LightGBMForecaster(BaseForecaster[ForecastExtraOutput,
    ...
    - ``None``（默认）：Direct 策略，为每个预测步长（以及每个目标维度）训练独立的
      LightGBM booster。
    - ``'MIMO'``：MIMO 策略，每个目标维度使用一个 LightGBM booster，以
    ...
    def __init__(self, *, oid: str | None = None, config: LightGBMForecasterConfig | None = None, **kwargs):
    ...
    def set_chunk_ids(self, chunk_ids: np.ndarray | None) -> None:
        """LightGBM 算子当前不支持 chunk_ids，传入时忽略并警告。"""
        if chunk_ids is not None:
            logger.warning(
                f"[{self.oid}] lightgbm_forecaster 不支持 chunk_ids，"
                "传入的 chunk_ids 将被忽略。"
            )
    ...
    def _fit_data(self, x: np.ndarray, y: np.ndarray, *, params: None) -> None:
        ...
        if self._is_mimo():
            for j in range(self._num_targets):
                y_train_j = Y_train[:, j]
                dtrain = lgb.Dataset(X_train, y_train_j)

                valid_sets = [dtrain]
                valid_names = ["train"]
                if X_val is not None and Y_val is not None:
                    y_val_j = Y_val[:, j]
                    dval = lgb.Dataset(X_val, y_val_j, reference=dtrain)
                    valid_sets.append(dval)
                    valid_names.append("val")

                logger.debug(
                    f"[{self.oid}] 训练 LightGBM MIMO booster: target={j}, "
                    f"n_train={n_train}"
                )
                self._models[j] = lgb.train(
                    base_params,
                    dtrain,
            ...
            for k in range(cfg.pred_len):
                for j in range(self._num_targets):
                    ...
                    dtrain = lgb.Dataset(X_train, y_train_kj)

                    valid_sets = [dtrain]
                    valid_names = ["train"]
                    if X_val is not None and Y_val is not None:
                        y_val_kj = Y_val[:, target_col]
                        dval = lgb.Dataset(X_val, y_val_kj, reference=dtrain)
                        valid_sets.append(dval)
                        valid_names.append("val")

                    logger.debug(
                        f"[{self.oid}] 训练 LightGBM booster: horizon={k}, target={j}, "
                        f"n_train={n_train}"
                    )
                    self._models[(k, j)] = lgb.train(
                        base_params,
                        dtrain,
                        num_boost_round=cfg.n_estimators,
                        valid_sets=valid_sets,
    ...
    def _run_data(self, x: np.ndarray, *, params: None) -> np.ndarray:
        ...
        if self._is_mimo():
            X_features = self._build_predict_features(x)  # (batch * pred_len, features)

            preds = np.empty((batch_size, cfg.pred_len, self._num_targets), dtype=np.float64)
            for j in range(self._num_targets):
                model = self._models[j]
                pred_j = model.predict(X_features)  # (batch * pred_len,)
            ...
            for k in range(cfg.pred_len):
                for j in range(self._num_targets):
                    ...
                    preds[:, k, j] = model.predict(X_features)
    ...
    def _save_fit_state(self, path: Path) -> None:
        ...
        if self._is_mimo():
            for j, model in self._models.items():
                model_path = model_dir / f"model_t{j}.txt"
                model.save_model(str(model_path))
        else:
            for (k, j), model in self._models.items():
                model_path = model_dir / f"model_h{k}_t{j}.txt"
                model.save_model(str(model_path))
    ...
    def _load_fit_state(self, path: Path) -> None:
        ...
        if self._is_mimo():
            for j in range(self._num_targets):
                model_path = model_dir / f"model_t{j}.txt"
                self._models[j] = lgb.Booster(model_file=str(model_path))
        else:
            for k in range(pred_len):
                for j in range(self._num_targets):
                    model_path = model_dir / f"model_h{k}_t{j}.txt"
                    self._models[(k, j)] = lgb.Booster(model_file=str(model_path))

        self._fitted = True


class LightGBMMIMOForecasterConfig(LightGBMForecasterConfig):
    """LightGBM MIMO 预测算子实例参数（兼容别名）。

    等价于 ``LightGBMForecasterConfig(strategy='MIMO')``，并保留 MIMO 默认超参。
    """
    ...
    num_leaves: int = Field(default=63, ge=1, le=1024, description="每棵树最大叶子数")

    class Config:
        extra = 'forbid'


class LightGBMMIMOForecaster(LightGBMForecaster):
    """LightGBM MIMO 时序预测算子（兼容别名）。

    等价于 ``LightGBMForecaster(config=LightGBMForecasterConfig(strategy='MIMO'))``。
    """

    _is_operator_alias: ClassVar[bool] = True
    _config_type: ClassVar[type[LightGBMMIMOForecasterConfig] | None] = LightGBMMIMOForecasterConfig

    @classmethod
    def name(cls) -> str:
        return "lightgbm_mimo_forecaster"
