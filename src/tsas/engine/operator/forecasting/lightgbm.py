...
基于 EPFAlgorithm 中的 LightGBMDirect 模型思想，采用 Direct 多步预测策略：
为每个预测步长（以及每个目标维度）独立训练一个 LightGBM booster。
...
None]):
    ...
    LightGBM booster。特征由输入历史窗口展平得到，适用于通用数值时序。
    ...
    def __init__(self, *, oid: str | None = None, config: LightGBMForecasterConfig | None = None, **kwargs):
        ...
        self._models: dict[tuple[int, int], lgb.Booster] = {}
    ...
    def _build_samples(self, x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        X_samples = np.empty((n_samples, cfg.seq_len * num_features), dtype=x.dtype)
        Y_samples = np.empty((n_samples, cfg.pred_len * num_targets), dtype=y.dtype)
