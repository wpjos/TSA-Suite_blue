...
基于 EPFAlgorithm 中的 XGBoostDirect 模型思想，采用 Direct 多步预测策略：
为每个预测步长（以及每个目标维度）独立训练一个 XGBoost booster。
...
None]):
    ...
    XGBoost booster。特征由输入历史窗口展平得到，适用于通用数值时序。
    ...
    def __init__(self, *, oid: str | None = None, config: XGBoostForecasterConfig | None = None, **kwargs):
        ...
        self._models: dict[tuple[int, int], xgb.Booster] = {}
