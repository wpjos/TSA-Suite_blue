...
__all__ = ["SODScorerConfig", "SODScorer", "SODDetectorConfig", "SODDetector"]


class SODScorerConfig(BaseModel):
    model_config = {"frozen": True}
    contamination: float = Field(default=0.1, gt=0, le=0.5)
    n_neighbors: int = Field(default=20, ge=1)
    ref_set: int = Field(default=10, ge=1)
...
class SODScorer(SingleScorerMixin[None], UnsupervisedNumericOperatorMixin[None], NumericOperator[None, SODScorerConfig, None]):
    ...
    def __init__(self, *, oid=None, config=None, **kwargs):
        ...
        self._model: SOD | None = None

    def _fit_data(self, x: np.ndarray, params: None) -> None:
        self._model = SOD(**self.config.model_dump())
        self._model.fit(x)
...
class SODDetectorConfig(SODScorerConfig):
    model_config = {"frozen": True}
    percentile: float = Field(default=95.0, ge=50.0, le=99.9)


class SODDetector(UnsupervisedNumericOperatorMixin[None], BaseDeciderMixin[None], NumericOperator[None, SODDetectorConfig, None]):
    @classmethod
    def name(cls) -> str:
        return "sod_detector"
    ...
    def __init__(self, *, oid=None, config=None, **kwargs):
        ...
        self._scorer = SODScorer(config=SODScorerConfig(**values))
    ...
    def _load_fit_state(self, path: Path) -> None:
        super()._load_fit_state(path)
        self._scorer = SODScorer.load(path / "_scorer")
