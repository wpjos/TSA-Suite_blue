...
算法逻辑源自对应公开实现的 ecod.ECOD, 按 TSA-Suite 算子规范重写。
不依赖 第三方库, 直接使用 pyod.models.ecod.ECOD。

核心思想: Empirical Cumulative Distribution-based Outlier Detection, 无参数。
"""
import pickle
...
__all__ = ['ECODScorerConfig', 'ECODScorer', 'ECODDetectorConfig', 'ECODDetector']


class ECODScorerConfig(BaseModel):
    """ECOD 评分器实例参数"""
...
class ECODScorer(SingleScorerMixin[None],
                 UnsupervisedNumericOperatorMixin[None],
                 NumericOperator[None, ECODScorerConfig, None]):
    """ECOD 直接评分器"""
    ...
    def __init__(self, *, oid=None, config=None, **kwargs):
        ...
        self._model: ECOD | None = None

    def _fit_data(self, x: np.ndarray, params: None) -> None:
        self._model = ECOD(contamination=self.config.contamination, n_jobs=self.config.n_jobs)
        self._model.fit(x)
...
class ECODDetector(UnsupervisedNumericOperatorMixin[None],
                   BaseDeciderMixin[None],
                   NumericOperator[None, ECODDetectorConfig, None]):
    """ECOD 检测器 = ECODScorer + PercentileDecider"""

    @classmethod
    def name(cls) -> str:
        return "ecod_detector"
    ...
    def __init__(self, *, oid=None, config=None, **kwargs):
        super().__init__(oid=oid, config=config, **kwargs)
        self._scorer = ECODScorer(config=ECODScorerConfig(
    ...
    def _load_fit_state(self, path: Path) -> None:
        super()._load_fit_state(path)
        self._scorer = ECODScorer.load(path / "_scorer")
