...
def _print_usage() -> None:
    ...
    print()
    print("通用子命令:")
    print("  list                  列出当前模块的所有可用算子")
    print("  show <算子名称>       查看指定算子的详细参数文档（可指定多个）")
    print("  run                   执行算子推理/计算")
    print("  fit                   执行算子训练（仅可训练算子）")
    print("  help [算子名称]       列出当前模块的所有可用算子或查看指定算子的详细参数文档 [已废弃，请使用 list 或 show]")
    print()
    print("示例:")
    print("  # 列出算子 / 查看详情")
    print("  python -m tsas.engine.operator.cli detection list")
    print("  python -m tsas.engine.operator.cli detection show knn_scorer")
    print("  # 直接推理（无需训练的算子）")
    print("  python -m tsas.engine.operator.cli evaluation run --input data.csv --config eval.json")
    print("  python -m tsas.engine.operator.cli feature_construction run --input data.csv --config feature.yaml")
    print("  # 首先训练并保存模型，随后加载模型进行推理")
    print("  python -m tsas.engine.operator.cli detection fit --input train.csv --config det.yaml --save model/")
    print("  python -m tsas.engine.operator.cli detection run"
          " --input test.csv --config det.yaml --load model/ --output result.csv")
