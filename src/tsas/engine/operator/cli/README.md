...
1. 继承了对应的基类或 Mixin：
   ...
   - 预测算子需继承 `BaseForecaster`
2. 实现了 `name()` 类方法，并返回了独一无二的名称。
3. 存放在对应模块的包或子包下。

> **依赖缺失时的行为**：如果某个算子模块因第三方依赖未安装（如 `lightgbm`、`xgboost`）导致 `ImportError`，该模块会被自动跳过并记录一条 warning 日志（包含模块名和失败原因），但不影响其他正常模块的注册。因此如果发现某算子未出现在 CLI HELP 列表中，请先检查日志中是否有相关的 warning 信息。
