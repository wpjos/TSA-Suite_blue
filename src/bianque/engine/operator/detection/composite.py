...
def _validate_operators(operators: list, *, require_decider: bool = False
    ...
    Returns:
        tuple[predictor, scorers, decider]: 解析后的子算子组件
            - predictor: BasePredictor 实例或 None
            - scorers: Scorer 列表
            - decider: BaseDeciderMixin 实例或 None
    ...
    for i, op in enumerate(operators):
        if isinstance(op, BasePredictor):
            if i != 0:
...
def _import_class(class_path: str):
    ...
    Args:
        class_path: 完整类路径，如 'bianque.engine.operator.detection.pca.PCAPredictor'
...
class CompositeScorerExtraOutput(BaseModel):
    ...
    Attributes:
        outputs: 子算子 EO 列表，与传入的 operators 顺序对应
    """
    outputs: list[BaseModel | None] = []
    """子算子 EO 列表"""
...
class CompositeScorer(
    ...
    @property
    def predictor(self) -> BasePredictor | None:
        """获取内部 Predictor 实例"""
...
class CompositeDetector(
    ...
    泛型参数:
        - EO: CompositeDetectorExtraOutput
        - C: None（无实例参数）
        - RP: None（无运行参数）
    """
