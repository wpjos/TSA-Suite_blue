...
HPO（超参数优化）能力已独立为 `bianque.engine.hpo` 模块，详见 `hpo/hpo.md`。
...
```
src/bianque/engine/operator/detection/
...
|----|--------------------------------------------------------------|-------|--------------------------------------|
| 1  | `BasePredictor`                                              | ✅     | 实现 `_fit_data` / `_run_data`，输出预测值   |
...
class AEPredictor(UnsupervisedNumericOperatorMixin[None],
                  BasePredictor[None, AEPredictorConfig, None]):
    def _fit_data(self, x: np.ndarray, params: None) -> None:
        # 训练 AutoEncoder...
        pass
...
```python
from bianque.engine.operator.detection.composite import CompositeScorer
...
可训练算子支持持久化。子类可覆写以保存额外状态（如模型权重），建议先调用 `super().save(path)`。
...
coverage report --include="src/bianque/engine/operator/detection/*" --show-missing
```
