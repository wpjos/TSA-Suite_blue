...
from bianque.engine.operator.base import LearnableOperatorMixin, C, NumericOperator, NumericData, \
    DataFrameMeta
...
class WindowFeatureConfig(BaseFeatureConfig):
    ...
    padding: Padding | float | int | None = Field(default=None)
    """
    填充模式:
        - ``None``: 不填充，输出行数 = 输入行数 - window_size + 1
        - ``Padding.EDGE``: 用边界值填充
        - ``Padding.NAN``: 用 NaN 填充
        - ``Padding.REFLECT``: 镜像填充
        - ``Padding.RING``: 循环填充（首尾相接）
        - ``float | int``: 用指定数值填充
    """

    alignment: Alignment = Field(default=Alignment.RIGHT)
    """
    窗口对齐方式:
        - ``Alignment.RIGHT``: 右对齐（默认），结果[i] 对应输入 x[i-w+1 : i+1]
        - ``Alignment.LEFT``: 左对齐，结果[i] 对应输入 x[i : i+w]
    """
