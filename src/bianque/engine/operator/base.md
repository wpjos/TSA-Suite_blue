...
2. **数值算子路线**（继承 `NumericOperator` 或 `BiNumericOperator`）：
    ...
    - 子类只需实现 `_run_data`（纯 ndarray 计算）和 `_name_output_columns`（输出列名）
...
```
_validate_input → _filter_data → _unwrap_data → _adjust_data
→ _run_data → _wrap_data（含 _adjust_index + _name_output_columns）
```
...
class ResidualScorer(BiNumericOperator[None, ResidualConfig, None]):
    ...
    def _run_data(self, x_real: np.ndarray, x_pred: np.ndarray, params) -> np.ndarray:
        diff = x_real - x_pred
