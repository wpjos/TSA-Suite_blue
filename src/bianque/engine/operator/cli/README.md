...
本文档旨在介绍如何使用 `bianque.engine.operator.cli` 提供的命令行工具来执行特征构造、时序异常检测和评价指标计算，以及如何将新开发的算子接入到 CLI 中生效。

## 1. 命令行调用入口

CLI 工具既支持通过 `bianque.engine.operator.cli` 作为统一入口（然后通过指定子模块名分发），也支持直接调用具体的模块入口。

**方式一：统一调用入口**

```bash
python -m bianque.engine.operator.cli <模块名> <子命令> [参数]
```
...
- **查看所有可用算子（以detection为例）：**
  ...
  **输出样例：**
  ...
  - **`knn_detector`**: 基于 K 近邻距离的异常检测器。
  - **`zscore_detector`**: 基于 Z-Score 的异常检测器。
  ...
  ```
...
- **查看指定算子的详细使用说明（以detection为例）：**
  ...
  **输出样例：**
  ...
  基于 K 近邻距离的异常检测器。计算每个样本到其第 k 个最近邻的距离作为异常分数，并基于百分位阈值进行决策。

  #### 实例参数 (Config)

   参数名 | 类型 | 必填 | 默认值 | 值域/候选 | 说明 |
  --------|------|------|--------|-----------|------|
   n_neighbors | int | 否 | 5 | [1, null] | 近邻数量 K |
   percentile | float | 否 | 95.0 | [0.0, 100.0] | 异常分数百分位阈值 |
...
```bash
python -m bianque.engine.operator.cli feature_construction run --input data.csv --config feature.yaml --output result.csv
或
python -m bianque.engine.operator.cli.feature_construction run --input data.csv --config feature.yaml --output result.csv
```
...
检测模块支持单个评分器 (`Scorer`)、决策器 (`Decider`) 或端到端检测器 (`Detector`)，也可通过 `composite_scorer`/`composite_detector` 编排管线。
...
```bash
python -m bianque.engine.operator.cli detection fit --input train.csv --config det.yaml --save model_dir/
```
...
python -m bianque.engine.operator.cli detection run --input test.csv --config det.yaml --load model_dir/ --output result.csv
```
...
```bash
python -m bianque.engine.operator.cli evaluation run --input data.csv --config eval.yaml --output result.json
```
...
当 CLI 启动时，相应的模块（如 `detection` CLI）会实例化一个 `OperatorRegistry`。注册中心会使用 `pkgutil.walk_packages` 自动递归扫描指定的包路径（如 `bianque.engine.operator.detection`）。
只要你的算子类满足以下条件，它就会在 CLI 中自动生效，**无需手动在某处注册**：

1. 继承了对应的基类或 Mixin：
    - 特征算子需继承 `BaseFeatureMixin`
    - 检测算子需继承 `BaseScorerMixin` 或 `BaseDetectorMixin`
...
```python
# src/bianque/engine/operator/detection/my_detector.py
from bianque.engine.operator.base import BaseOperator
from bianque.engine.operator.detection.base import BaseDetectorMixin
...
class MyDetector(BaseDetectorMixin, BaseOperator[..., MyDetectorConfig, ...]):
...
**Step 3. 验证生效：**
确保文件放在相应的目录下（例如 `src/bianque/engine/operator/detection` ）。运行以下命令：

```bash
python -m bianque.engine.operator.cli detection help my_detector
```
