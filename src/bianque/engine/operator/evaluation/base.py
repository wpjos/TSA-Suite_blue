...
使用示例::
    ...
    from bianque.engine.operator.evaluation.base_metric import (
...
from bianque.engine.operator.base import BaseOperator, I, RP
...
class BaseMetricOperator(
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        使用 ``BaseOperator._extract_type_from_typevar`` 从 MC 泛型参数提取 Config 类型，
        然后验证其必须是 BaseMetricConfig 的子类。
        ...
        # 但 BaseMetricOperator 使用 MC 替换了 C，导致 BaseOperator 无法提取。
        # 通过 BaseOperator._extract_type_from_typevar 从 MC 泛型参数中提取。
        if cls._config_type is None:
            cls._config_type = BaseOperator._extract_type_from_typevar(cls, BaseMetricOperator, MC)
