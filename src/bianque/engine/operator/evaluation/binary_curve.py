...
class BinaryClassificationCurveResult(BaseModel):
    ...
    best_specificity_threshold: float
    best_f1: float
    best_f1_threshold: float
    best_mcc: float
    best_mcc_threshold: float
    best_tpr: float
    best_tpr_threshold: float

    # 各指标最优值（越小越好）及对应阈值
    best_fpr: float
    best_fpr_threshold: float
    best_far: float
    best_far_threshold: float
    best_mar: float
    best_mar_threshold: float

    # 曲线数组
    thresholds: list[float]
    tpr: list[float]
    fpr: list[float]
    precision_arr: list[float]
...
class BinaryClassificationCurveConfig(BaseMetricConfig):
    ...
    default_for_zero: float = 0.0
    decimals: int = 6
    predict_decimals: int = 0
    distinct_thresholds: bool = True
    inf_threshold: bool = True
    main_scores: dict[str, str] | None = {"auc_roc": "auc_roc", "best_f1": "best_f1"}
