...
本文档旨在指导开发者基于 `base_metric.py` 框架开发新的评价指标算子，并提供已实现算子的使用参考。

**目录结构**：

```
src/bianque/engine/operator/evaluation/
├── __init__.py             # 包导出（MR, MC, BaseMetricConfig, BaseMetricOperator）
├── base_metric.py          # 基础类型定义（开发核心）
...
```python
from bianque.engine.operator.evaluation import BinaryClassificationMetric
...
```python
from bianque.engine.operator.evaluation import BinaryClassificationCurve
...
# HPO 集成
op = BinaryClassificationCurve(main_scores={"auc": "auc_roc", "f1": "best_f1"})
...
```python
from bianque.engine.operator.evaluation import MultipleClassificationMetric
...
print(result.label_metrics[0].f1)  # 第 0 类的 F1
...
```python
from bianque.engine.operator.evaluation import PointAdjust
...
```python
from bianque.engine.operator.evaluation import SelfEvaluation
...
```python
from bianque.engine.operator.evaluation import BaseMetricConfig
...
```python
import numpy as np
from bianque.engine.operator.evaluation import BaseMetricConfig, BaseMetricOperator
...
```python
from bianque.engine.operator.evaluation import BaseMetricOperator
...
from bianque.engine.operator.evaluation import BaseMetricOperator, BaseMetricConfig
...
# 覆盖 Config 默认值
op = MyMetricOp(main_scores={"f1": "f1"})

# 传入自定义 Config 实例
op = MyMetricOp(config=MyConfig(main_scores={"f1": "f1"}, positive_label=1))

# 显式指定名称
op = MyMetricOp(name="my_metric", main_scores={"f1": "f1"})
```
...
```bash
python -m pytest test/test_engine_operator/test_evaluation/ --cov=bianque.engine.operator.evaluation.base_metric --cov-report=term-missing -q
```
