...
"""
LightGBM MIMO 时序预测算子单元测试

对应源文件：
- forecasting/lightgbm_mimo.py: LightGBMMIMOForecaster, LightGBMMIMOForecasterConfig
...
class TestLightGBMMIMOForecaster:
    ...
    def test_fit_learns_model(self, train_data, minimal_config):
        """目的：验证 fit 后已训练 booster"""
        forecaster = LightGBMMIMOForecaster(config=minimal_config)
        y = train_data[:, [-1]]
